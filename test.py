...
def photo():
	...
	ip2 = ''  # Modifier ici l'adresse ip de la camera IP
...
#---- Prise des photos ff=camera_usb ff_ip=camera ip
	while len(yeux)!=2 and len(yeux_ip)!=2 and countTimeout!=5:
		countTimeout += 1
		ff = grab_frame2()
...
#----Traitement de la photo de la caméra_IP1
	...
	photo_tmp = open("photo_tmp.txt", "r")
	value = photo_tmp.read()
#----Création du fichier pour autoriser la fonction On_off
	on = open('on_off.txt', 'w')
	if len(yeux)==2 or len(yeux_ip)==2:
		on.write('1')
	else:
		on.write('0')
...
#		cam.write('2')
	...
	if value == 1:
		os.system('cp /home/pi/Downloads/bonnePhoto.png /home/pi/Downloads/eyesight/static/1.png')
	else:
		os.system('cp /home/pi/Downloads/bonnePhoto.png /home/pi/Downloads/eyesight/static/0.png')
	photo_tmp.close()